...
- 项目默认使用本地 MySQL、Redis 和 Nacos，运行前请先启动相关服务。
- 数据库密码、客户端 AK/SK 等配置通过环境变量传入，示例配置仅用于本地演示。
- 如需在自己的环境部署，请根据实际情况修改数据库、Redis、Nacos 和网关路由配置。
- 接口文档可在服务启动后通过 Knife4j / Swagger 相关地址查看。
